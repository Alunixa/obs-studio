...
- 之前已发布源码为 `87c9da6d81e14a40af2486496b1b4397242d16c8`；本轮尚未重新核验安装版本。
...
## 14. Pending Work
- 核对实际运行版本、近期错误/编码日志、当前缓存实际占用及保存状态。
- 审计长时间窗口回收、失败后的冻结、稀疏失败、索引/快照并发、异常退出残留。
- 为证实缺陷增加先失败后通过的回归，独立编译验证，再决定发布。
- 花屏需要具体样本/时间点或可复现的动态测试；不将可解码等同视觉无异常。

## 15. Known Bugs and Limitations
- 新报告：花屏持续，保存约 9 GB 回放后缓存可能约 40 GB；尚未确定原因。
- 既有测试曾在并行 GPU 负载下出现严重编码跳帧/排空慢，未证明性能全面通过。
- 保存失败后的数据保护与容量约束可能影响增长，需重新审计，暂不作为已确认根因。
...
- 当前阶段：只读调查前准备；尚未改变源码、运行实例、私人设置或媒体。

## 20. Next Steps
1. 读回本文件，创建本轮分支和检查点，追加 `YHYQ.md`。
2. 核对实际进程、最近日志、配置中的回放窗口/大小/编码、缓存分配空间。
3. 阅读存储/回放实现与测试，重现边界和失败生命周期。
4. 仅修复可证实问题，逐步记录、提交、测试和交付状态。
