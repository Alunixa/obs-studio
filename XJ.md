...
- 当前用户正在打游戏：本轮禁止本机编译、编码/解码测试和额外 GPU/重型负载，使用 GitHub Actions 远程构建交付 ZIP；不自动下载大包或解压安装，以免影响游戏。
...
- 用户补充 VLC 几乎所有视频多处花屏/偏色，并明确要求现在用 Actions 构建 ZIP；发包准备检查点 `d4d095b61`，仅调整发布配置和说明，不新增未经验证的编码改动。
...
- 已按用户明确要求准备远程发包：工作流增加当前修复分支触发，版本 `32.2.2-alunixa.20260915.1`，独立标签 `alunixa-20260915-<short-sha>`，说明读取 `docs/alunixa-replay-cache-20260915.md`；目前待推送触发 CI。
- 本轮只提供 Actions 发布 ZIP 和校验信息，不自动下载大 ZIP 到正在游戏的电脑、不覆盖安装；远程构建和发布必须实际成功后才能交付。
...
- 检查发布工作流脚本语法，提交并推送当前分支，等待并验收 Actions / Release ZIP 与 SHA-256；不要启动本机构建。
...
- 最新用户反馈：播放器为 VLC，几乎所有视频多处花屏或偏色；花屏仍未定位，本轮缓存修复的 Release 不宣称修复此问题。
...
- 当前阶段：根据用户要求进行 Actions-only 发包，发布前检查点 `d4d095b61`；已核对远程没有在运行的旧构建、留档 Release 已存在，正在调整工作流与说明，尚未触发新构建。花屏未定位，用户运行环境不动。

## 20. Next Steps
1. 轻量检查工作流脚本并提交，推送当前修复分支触发远程 CI。
2. 等待 GitHub Actions 结束，失败则查远程日志修正，不重复启动已在运行的构建。
3. 验证 Release 对应提交、ZIP 资产、SHA-256 和回归结果，交付直接下载链接，不在用户电脑下载/解压大包。
4. 花屏后续再定位；保持现有编码设置、OBS 与游戏进程不变，不绕过之前的清理拦截。
...
- 2026-09-15：用户确认 VLC 普遍花屏/偏色，要求只用 Actions 构建 ZIP、不要占用正在游戏的电脑；已核对项目记忆、GitHub 权限和构建状态，创建 `d4d095b61` 检查点并准备 `20260915.1` 发布工作流。
